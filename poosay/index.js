...
console.log('🔍 [DEBUG] เริ่มโปรแกรม index.js');
console.log(`🔍 [DEBUG] DISCORD_TOKEN: ${process.env.DISCORD_TOKEN ? 'มีค่า ยาว ' + process.env.DISCORD_TOKEN.length + ' ตัวอักษร' : '❌ ไม่มีค่าเลย (undefined)'}`);
if (process.env.DISCORD_TOKEN) {
  const t = process.env.DISCORD_TOKEN;
  const trimmed = t.trim();
  console.log(`🔍 [DEBUG] TOKEN หลัง trim() เหลือ ${trimmed.length} ตัวอักษร (ถ้าตัวเลขไม่เท่าเดิม แปลว่ามีช่องว่าง/newline ติดมา)`);
  console.log(`🔍 [DEBUG] TOKEN ขึ้นต้นด้วย: "${t.slice(0, 6)}" ลงท้ายด้วย: "${t.slice(-6)}"`);
  console.log(`🔍 [DEBUG] จำนวนจุด (.) ใน token: ${(t.match(/\./g) || []).length} (token ปกติต้องมี 2 จุด)`);
}
console.log(`🔍 [DEBUG] CLIENT_ID: ${process.env.CLIENT_ID || '❌ ไม่มีค่าเลย'}`);
...
console.log('🔍 [DEBUG] กำลังโหลด discord.js และไฟล์อื่นๆ...');
...
console.log('🔍 [DEBUG] โหลดไฟล์ทั้งหมดสำเร็จ กำลังสร้าง Client...');
...
console.log('🔍 [DEBUG] สร้าง Client สำเร็จ กำลังตั้งค่า event listeners...');
...
if (!process.env.DISCORD_TOKEN) {
  console.error('❌ ไม่พบ DISCORD_TOKEN — เช็คว่าตั้งค่า Environment Variable นี้ไว้แล้วหรือยัง');
  process.exit(1);
}

console.log('🔍 [DEBUG] กำลังเรียก client.login()...');

const loginWatchdog = setTimeout(() => {
  console.error('⚠️ [TIMEOUT] ผ่านไป 20 วินาทีแล้วยังไม่ ready — Discord gateway อาจเชื่อมต่อไม่ได้ หรือ intent ไม่ถูกเปิดใน Developer Portal');
}, 20000);

client.once('ready', () => clearTimeout(loginWatchdog));

client.login(process.env.DISCORD_TOKEN)
  .then(() => {
    console.log('🔍 [DEBUG] client.login() resolve แล้ว (แปลว่า token ผ่าน แต่รอ event ready อยู่)');
  })
  .catch((err) => {
